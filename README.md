...
<style>
    ...
    https://github.com/bookingluxury/gaigoi/tree/main
</style>
...
<body>
<div id="region-section" class="content active">
    <h2>Thành phố (대도시)</h2>
    <div class="region-list">
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Seoul
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Busan
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Incheon
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Daegu
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Daejeon
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Gwangju
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Ulsan
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Sejong
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
            <i class="fas fa-map-marker-alt"></i> Pohang
        </div>
        <div class="region-btn" onclick="openLinkInNewTab()">
...
<html lang="en">
  <head>
    ...
    <style>
      .support-icon {
        ...
        color: #fff;
        border-radius: 50%;
        background-image: url("https://i.ibb.co/XFYWHm5/pngtree-support-icon-modern-png-image-10723665.png"); /* Thay thế thành đường dẫn ảnh avatar */
      ...
      .support-chat {
        ...
        width: 300px;
        height: 500px;
        background-color: linear-gradient(45deg, #ff7eb9, #ff65a3);
      ...
      .support-chat-buttons button,
      .support-chat-buttons input[type="file"] {
      ...
      .ban-message {
        ...
        font-weight: 600; /* Chữ đậm hơn */
        border-radius: 8px;
        text-align: right; /* Tin nhắn người dùng nằm bên phải */
      }

      .user-avatar {
        align-self: flex-end; /* Align avatar to the right */
        width: 30px;
        height: 30px;
        border-radius: 50%;
        margin-left: 10px;
  ...
  <body>
    <!-- Support icon and chat structure -->
    <div
      class="support-icon"
      id="supportIcon"
    ...
    <div class="support-chat" id="supportChat">
      <div class="support-chat-header">Support Chat</div>
      ...
      <div class="support-chat-buttons">
        ...
        <button onclick="sendImageToTelegram()">Send Image</button>
    ...
    <script>
      ...
      function startDrag(event) {
        isDragging = true;
        offsetX = event.clientX - supportIcon.offsetLeft;
        offsetY = event.clientY - supportIcon.offsetTop;
        document.addEventListener("mousemove", dragIcon);
        document.addEventListener("mouseup", stopDrag);
      }

      // Drag the icon around the page
      function dragIcon(event) {
        if (isDragging) {
          const x = event.clientX - offsetX;
          const y = event.clientY - offsetY;
          supportIcon.style.left = x + "px";
          supportIcon.style.top = y + "px";
        }
      }
      ...
      function toggleSupportChat() {
        ...
        supportChat.style.display =
          supportChat.style.display === "flex" ? "none" : "flex";
      }
  ...
  chat_responses: {
    greeting: {
      user: "Chào em",
      bot: "Dạ, em chào anh yêu ạ! Em có thể giúp gì cho anh yêu hôm nay không? Anh yêu muốn tìm hiểu dịch vụ hay có thắc mắc gì cần em giải đáp không ạ?",
    },
    book_girl: {
      user: "Anh muốn book gái",
      bot: "Dạ, em hiểu rồi, anh yêu muốn book gái. Anh yêu cần em tư vấn thêm về khu vực, giá cả, hay loại hình dịch vụ cụ thể nào không ạ?",
    },
    location: {
      user: "Ở khu vực nào có?",
      bot: "Dạ, hệ thống hiện có các bé ở nhiều khu vực khác nhau để phục vụ anh yêu ạ! Anh yêu có thể cho em biết mình đang ở khu vực nào không, để em giới thiệu các bé gần anh nhất nhé?",
    },
    price: {
      user: "Giá cả thế nào?",
      bot: `Dạ, em xin giới thiệu các gói dịch vụ hiện có để anh yêu dễ chọn hoặc anh có thể xem ở mục bảng giá cho chi tiết nhé:
      \n\n**Gói Thường**: 100,000 won
      Thời gian: 90 phút
      Dịch vụ: Tắm chung, massage thư giãn, HJ & BJ chuyên nghiệp, quan hệ 2 lần
      \n\n**Gói VIP**:145,000 won
      Thời gian: 180 phút
      Dịch vụ: Massage kết hợp, HJ & BJ nóng/lạnh, quan hệ 3 lần, quay lại khoảnh khắc
      \n\n**Gói Luxury**:335,000 won
      Thời gian: 16 giờ
      Dịch vụ: Tắm chung, đồ chơi cao cấp, massage Nuru, quan hệ không giới hạn
      \n\n**Gói VIP Tour**:1,900,000 won
      Thời gian: 3 ngày
      Dịch vụ: Phục vụ toàn diện, quay khoảnh khắc, phục vụ tận tình như người yêu
      \n\nAnh yêu có thể chọn gói nào phù hợp, hoặc nếu cần thêm thông tin, cứ báo em nhé!`,
    },
    payment: {
      user: "Nếu anh chuyển khoản trước mà bị mất thì sao?",
      bot: "Dạ, em rất hiểu nỗi lo này. Hệ thống luôn cam kết minh bạch với tất cả các khoản thanh toán. Khoản phí 19,000 won chỉ là phí đăng ký mã đặt lịch và sẽ được tính vào chi phí dịch vụ của anh yêu. Nếu anh không sử dụng nữa, khoản này có thể hoàn lại hoặc dùng làm tín dụng cho lần đặt lịch sau. Anh cứ yên tâm nhé!",
    },
    registration_process: {
      user: "Làm sao để đăng ký?",
      bot: "Dạ, để đăng ký, anh yêu cần thực hiện các bước sau:\n1. Đăng ký tài khoản trên hệ thống.\n2. Thanh toán khoản phí đăng ký 19,000 won.\n3. Nhận mã đặt lịch sau khi thanh toán thành công. Mã này sẽ được dùng để xác nhận và dễ dàng đặt lịch với các bé.",
    },
    phone_number: {
      user: "Cho anh số điện thoại bé được không?",
      bot: "Dạ, để đảm bảo quyền riêng tư và sự an toàn cho cả anh và các bé, bên em không cung cấp trực tiếp số điện thoại. Sau khi anh hoàn tất đăng ký mã đặt lịch, anh có thể liên hệ trực tiếp qua hệ thống Telegram. Anh yêu cứ yên tâm, em sẽ hỗ trợ anh hết mình!",
    },
  },
  process: {
    step1: {
      description: "Khách hàng thanh toán khoản phí đăng ký 19,000 won để nhận mã đặt lịch.",
      action: "Khách hàng gửi chứng từ thanh toán (ảnh chụp màn hình hoặc thông tin giao dịch) cho hệ thống.",
    },
    step2: {
      description: "Xác nhận thanh toán.",
      action: "Hệ thống sẽ xác nhận thanh toán của khách hàng và lưu lại thông tin giao dịch.",
    },
    step3: {
      description: "Cung cấp mã đặt lịch.",
      action: "Sau khi thanh toán được xác nhận, hệ thống sẽ gửi mã đặt lịch qua một kênh riêng tư.",
    },
    step4: {
      description: "Sử dụng mã đặt lịch.",
      action: "Khách hàng dùng mã đặt lịch để chọn bé yêu thích, gói dịch vụ và hoàn tất lịch hẹn.",
    },
  },
  payment_methods: {
    bank_accounts: {
      hana_bank: {
        account_number: "51291072352007",
        account_holder: "Pham Van Trung",
      },
      tpbank: {
        account_number: "00006001701",
        account_holder: "Bùi Thị Mỹ An",
      },
      shb: {
        account_number: "1023606558",
        account_holder: "Bùi Thị Mỹ An",
      },
    },
    payment_instruction: "Sau khi thanh toán, anh yêu gửi ảnh xác nhận để hệ thống kích hoạt mã đặt lịch.",
  },
  links: {
    registration_link: "https://girlluxury.club",
    referral_code: "369268",
  },
  privacy_and_security: {
    disclosure: "Mọi thông tin và mã đặt lịch sẽ được bảo mật tuyệt đối. Anh yêu không cần lo lắng về việc lộ thông tin cá nhân.",
  },
  important_notes: {
    note1: "Khoản phí 19,000 won không phải là phí dịch vụ mà là phí đăng ký mã đặt lịch.",
    note2: "Sau khi nhận mã, anh yêu có thể đặt lịch cho bất kỳ bé nào trong hệ thống mà không cần đăng ký lại.",
  },
};


// Hàm tải dữ liệu từ chatbot_update.json
async function loadChatbotData() {
  try {
    const response = await fetch("chatbot_update.json");
    if (!response.ok) {
      throw new Error("Không thể tải chatbot_update.json");
    }
    const data = await response.json();
    dulieubot.chat_responses = data.chat_responses;
    dulieubot.process = data.process;
    console.log("Dữ liệu JSON đã được tải:", dulieubot);
  } catch (error) {
    console.error("Lỗi khi tải chatbot_update.json:", error);
  }
}

// Gọi loadChatbotData khi trang được tải
window.onload = loadChatbotData;


      // Toggle the support chat visibility
      function toggleSupportChat() {
        const supportChat = document.getElementById("supportChat");
        supportChat.style.display =
          supportChat.style.display === "flex" ? "none" : "flex";
      ...
      function displayMessage(sender, message) {
        ...
        // Add custom CSS class based on sender (user or AI)
        messageElement.classList.add(
          sender === "Bạn" ? "ban-message" : "ho-tro-message"
        );
      ...
      // Kiểm tra câu hỏi và trả lời từ dulieubot.json hoặc sử dụng OpenAI API
      async function sendSupportMessage() {
        ...
        // Trả lời từ dulieubot.json nếu có từ khóa
        let responseMessage = "";

        // Các câu hỏi từ dulieubot.json
        if (message.toLowerCase().includes("giá")) {
          responseMessage = dulieubot["chat_responses"]["price"]["bot"];
        } else if (message.toLowerCase().includes("book gái")) {
          responseMessage = dulieubot["chat_responses"]["book_girl"]["bot"];
        } else if (message.toLowerCase().includes("chào em")) {
          responseMessage = dulieubot["chat_responses"]["greeting"]["bot"];
        } else {
          // Nếu không có từ khóa, gửi yêu cầu tới OpenAI API
          try {
                  ...
                  messages: [
                    {
                      role: "system",
                      content:
                        "Bạn là một trợ lý hỗ trợ khách hàng trong lĩnh vực dịch vụ book gái gọi.Khách muốn thông tin các bé thì bắt buộc xin địa chỉ trước.Khi khách gửi địa chỉ thì yêu cầu khách vào mục gần đây để check thông tin các bé. Dịch vụ tại Hàn quốc chỉ có các cô gái Việt nam không có các quốc giá khác,đa số là du học sinh hay thực tập sinh.Chỉ được chào anh yêu 1 lần không được lặp lại, trả lời đúng trọng tâm,Tên hệ thống là Luxury Girl Booking hoạt động 3 năm, Hạn chế gọi Luxury Girl Booking tránh làm phiền khách hàng.Bạn phải luôn xưng 'em' với khách hàng và 'anh yêu' với người dùng.Không bao giờ được xưng tôi dù trường hợp nào, hãy quan tâm hỏi han khách hàng như tình nhân ăn uống hay công việc nhưng không được lộ liễu,phải thật ngọt ngào,hạn chế chào nhiều lần lặp lại,",
                    },
                    {
                      role: "user",
                      content: message,
            ...
            // Trả lời từ OpenAI nếu không có trong dulieubot.json
            responseMessage = data.choices[0].message.content;
          } catch (error) {
            console.error("Error:", error);
            responseMessage = "Dạ anh yêu";
          }
        }

        // Hiển thị câu trả lời từ Hỗ Trợ
        displayMessage("Hỗ Trợ", responseMessage);
      }
      ...
      async function sendImageToTelegram() {
        ...
        const chatId = "8097050355"; // Replace with recipient's chat ID
        const botToken = "7957918125:AAG6hRCr5FMfq8sPSL44WF93qxvQRs_oQSw"; // Replace with your bot's token
        ...
        try {
          ...
          if (result.ok) {
            ...
            displayMessage(
              "Hỗ Trợ",
              "Hóa đơn của anh đã được nhận và đang xử lý."
            );
          } else {
